...
func store() -> void:
	...
	for module in _modules_map.node_name_by_module:
		var node_name = _modules_map.node_name_by_module[module]
		var node = _modules_map.get_node(node_name) as GraphNode
...
func restore() -> void:
	...
	while file.get_position() < file.get_length():
		...
		if not _modules_map.has_node(module_node_name):
			continue
		
		var node = _modules_map.get_node(module_node_name) as GraphNode
